...
const columnsOther = reactive([{
  type: 'input',
  label: global.$t('地区'),
  placeholder: global.$t('地区'),
  validateFirst: true,
  inputAlign: 'right',
  readonly: true,
  prop: 'inte_region_id',
...
function initData() {
  ...
  columnsOther[0].placeholder = dicts?.regionlist?.filter((item) => item.id === user?.userInfo?.inte_region_id)[0]?.name || global.$t('地区');
  columnsOther[1].placeholder = dicts?.regionlist?.filter((item) => item.id === user?.userInfo?.inte_region_id)[0]?.currency_conf?.list?.filter((child) => child.id === user?.userInfo?.inte_currency_id)[0].name || global.$t('货币');

  columns[0].disabled = !!user?.userInfo?.realname;
  columns[1].disabled = !!user?.userInfo?.birthday;
  ...
  if (columns[0].disabled && columns[1].disabled) showBtn.value = false;
}
onMounted(() => {
  initData();
});
