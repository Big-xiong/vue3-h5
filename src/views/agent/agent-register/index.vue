...
import {
  computed, inject, reactive, ref, watchEffect, onMounted,
} from 'vue';
...
const { userInfo } = global.$store.state.agent;
const columns = reactive([
  ...
  },
  {
    ...
    block: true,
    label: global.$t('登录密码'),
    placeholder: global.$t('密码6-20位数字及字母组成'),
  },
  {
    type: 'password',
    prop: 'repassword',
  ...
  },
  {
    ...
    block: true,
    label: global.$t('佣金比例'),
    placeholder: global.$t('佣金比例不得超过自身比例'),
  },
]);
...
const { run: register, loading: registerLoading } = useRequest(
  () => addagent(FormRef.value.data),
  {
    manual: true,
    onSuccess() {
      global.$dialog({ tpl: 'AgentTips', message: global.$t('代理开户成功') });
...
function doRegister() {
  ...
  if (data.rate < 0 || data.rate > Number(userInfo?.config?.commission?.rate)) {
    Toast.fail(global.$t('佣金比例不能小于0且不得超过自身比例'));
    return;
  }
  if (!/^[+]{0,1}(\d+)$/.test(data.rate)) {
