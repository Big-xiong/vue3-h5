<template>
  <div class="slots">
    <!-- 最新爆奖 -->
    <div class="slots-top">
      <div ref="bannerRef" class="slot-banner">
        <van-image :src="bannerPic ?? $getSrc('/src/current_project/assets/images/home/slots/banner.png')"
          class="banner"></van-image>
      </div>
      <div class="sss">
        <div class="jackpot">
        <img :src="$getSrc('/src/current_project/assets/images/home/slots/light@2x.png')" class="light" />
        <img :src="$getSrc('/src/current_project/assets/images/home/slots/light2@2x.png')" class="light2" />
        <div class="jackpot-text" v-if="state.jackpotSettings?.startVal">
          <h3>
            ...
            <count-to :duration="state.jackpotSettings?.duration" :decimals="state.jackpotSettings?.decimals"
              :startVal="state.jackpotSettings?.startVal" :endVal="state.jackpotSettings?.endVal" />
          </h3>
          <p>{{ $t("奖金池数值") }}</p>
        </div>
      </div>
    ...
    <van-sticky :offset-top="0" class="header-sticky">
      <nav class="filter-nav">
        <van-tabs v-model="active" line-height="2px" :ellipsis="false" @click="onTabChange" swipeable>
          <van-tab v-for="item in navs" :title="$t(item.title)" :key="item.name" />
        </van-tabs>
        <div class="search">
          <SearchTrigger class="seach-icon" :category="state.cateId" :nav="nav" :payforline="payforline"
            :platform="platform" />
          <span>{{ $t('搜索') }}</span>
        </div>
      </nav>
      ...
      <div class="filters" v-show="active !== 4">
        <div class="labels platform">
          <div>
            <label class="labels-all" :class="{ active: !platform }" @click="onPlatformClick(null)">{{
                $t('全部平台')
            }}</label>
            <template v-for="(item, index) in platformsSlot">
              <label v-if="['ALLBET'].indexOf(item.name) === -1" :key="index"
                :class="{ active: platform && platform.id === item.id }" @click="onPlatformClick(item)">{{
                    item?.code ?
        ...
        <div class="labels">
          <label :class="{ active: !payforline }" @click="onPayforlineClick(null)">{{ $t('全部赔付线') }}</label>
          <label v-for="(val, key) in state.payforlineData" :key="key" :class="{ active: key === payforline }"
            @click="onPayforlineClick(key)">{{ val.replace("线", "") }}</label>
        </div>
      </div>
    </van-sticky>
    <van-list class="games" :class="active === 4 ? 'mysave' : ''" v-model="loading" :finished="finished"
      :finished-text="lists?.length ? $t('不要滑了，已经到底了！') : ''" @load="onLoad" :immediate-check="false">
      <template v-if="lists?.length">
        <van-cell v-for="(item, index) in lists" :key="index">
          <div class="game-item" slot="default">
            ...
            <span class="action">
              <van-icon @click="doFavorite(item.id, index)" :name="
                item.is_favorite === 2 ? 'like-o' : user?.isLogin ? 'like' : 'like-o'
              " />
            </span>
          </div>
        </van-cell>
...
function resetScrollTop() {
  // bannerRef.value.scrollTop = 0;
  nextTick(() => {
    bannerRef.value?.scrollIntoView?.({ behavior: 'smooth' });
  });
}
...
.jackpot {
  ...
  margin: 0 auto 30px;
  width: 688px;
  background: var(--theme-color) url("/src/assets/images/home/slots/jackpot-bg@2x.png") no-repeat;
