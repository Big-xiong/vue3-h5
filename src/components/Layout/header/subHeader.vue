...
watch(() => router.currentRoute, () => {
  ...
  // 电子内页如果从平台进入则显示平台名称
  if (router.currentRoute.value.meta.title === '电子' && router.currentRoute.value.query.plat) {
    title.value = router.currentRoute.value.query.plat;
  }
...
function handleClick() {
  if (meta.value.go) {
    router.push({ name: meta.value?.go });
  } else {
    route.name === 'agentLanding' ? router.push('/') : router.go(-1);
  }
}
